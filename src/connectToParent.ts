...
export function connectToParent<TCallSender extends CallSender>(
  ...
  // 处理握手 Syn-Ack 消息
  const handleSynAckMessage = (data: HandshakeMessage) => {
    ...
    zLog(`握手 - 收到SYN-ACK消息, 回应ACK消息: ${JSON.stringify(ackMessage)}`);
    sendData(ackMessage);
  ...
  const handleMessageEvent = ({ origin: remoteOrigin, data }: MessageEvent) => {
    ...
    const { penpal } = data;
    if (penpal === MessageType.SynAck) {
      handleSynAckMessage(data);
